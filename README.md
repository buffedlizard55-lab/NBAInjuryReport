...
| Team coverage gaps | Every refresh names the teams the current snapshot says **nothing** about (last committed snapshot **2026-09-20T12:20:59Z**: still 27 of 30 — CLE, DET and LAL absent, each as an empty array; re-read 2026-09-19 on the live API the same three). Each linked to that team's own ESPN injuries page. A 30-chip strip and, in the unfiltered team-grid, empty franchise cards make the same three visible instead of only naming them in a coverage line. An omitted block is never reported as a healthy roster. Snapshot age is now printed on the dashboard and warns when >24 h stale. |
| Season clock | Dated public pages, not a guess: overseas camp **2026-09-22 (today on 2026-09-22 — LIVE)**, rest of league 2026-09-29 (7d), preseason 2026-10-03 (11d), opening night 2026-10-20 (28d). ESPN/NBA list the opener at 3pm / 7pm / 9:30pm ET; Basketball Monster lists 2:00pm / 6:00pm / 8:30pm — both claims are shown. Season-clock today-banner now highlights the gate, pinned by smoke check on 2026-09-22T12:00Z. |
...
| Social intelligence / forward score | Automatic post ledger, one-player exact-name resolution, original text/URL/hash, posted/first-observed times. Claims reconcile against **two layers, official first**: a game-date-matched NBA report designation (corroboration 3 pts / conflict-review), then the ESPN structured board — accepted only when **both** our sighting time and ESPN's own status stamp provably postdate the post, within 72 h (board agreement 1 pt, never official confirmation). A maturing official designation overrides a board outcome, never the reverse. The **forward score** sums agreement only; conflicts are review-only and never subtract; silence is not a verdict. **Agreement is not accuracy: no historical scorecards or global first-to-report rankings.** In-game claims stay pending without game-specific outcomes. |
| Reporter directory / in-arena layer | Now **computed per team** (`arenaCoverage()` in `assets/js/data.js`), not a typed table. Re-measured **2026-09-21T15:53:09Z** (`data/live/reporter_verify.json` — 94 handles checked: 70 ok / 24 dormant, in-alert-path 78 = **54 active / 24 dormant / 0 unmeasured**, 30-day rule; previous 2026-09-19 snapshot was 47/18 before the daily CI run measured activity drift). Team split on 2026-09-19: **18 of 30 teams Bluesky-verified pollable, 12 bio-verified pollable, 0 official-only, 0 gaps** — the 12 short of verification are named as gaps, never rendered as covered. Now 78 pollable in alert path, 21 Bluesky-verified writers. Two honest dead ends: Charlotte's paper of record eliminated its Hornets beat on 2026-09-14, so CHA's supported identities are a podcast analyst (`bgeisinger`) and a culture-site journalist (`britishbuzz`) — the name-matched `rodboone` handle has no bio and is REFUSED — and UTA, which lost its writer to MIN in session 13, reads **dormant-only**. Every row stores the verbatim bio, the re-checkable API URL, and the counts observed that day. A verified account is not a verified claim, a tier is not an accuracy score, and an assigned beat does not prove attendance. |
| Identity **and** activity | `arenaCoverage()` returns a per-team `recency` state and a per-writer `recency` object, computed from the newest post actually measured (`ARENA_DORMANT_DAYS` = 30). Identity answers *who*; recency answers *whether anyone is still posting*. Session 14 closed the "30 of 40 writers unmeasured" hole at both ends: `tools/backfill_registry_recency.js` copies the measured date back into every registry row (and `live-audit.yml` now runs it and fails a run where the registry is behind), while six `getAuthorFeed?limit=1` reads measured the rows that had never been read. Result — **the page reads identically with and without the CI evidence file**: 47 writers active, 18 dormant, **0 unmeasured**; 27 teams with an active writer; **DAL, LAL and UTA dormant-only and labelled as such**; CLE (Danny Cunningham, −17d) and DEN (Joel Rush, −2d) both leave the dormant list on measurement, not on hope. Boundaries the numbers keep: a dormant row is kept, not deleted, because the identity is still evidenced; an ACTIVE feed still has to clear the injury-vocabulary gate before it can alert; and the quietest pollable writer (Jason Lloyd, CLE, 638 days) is printed, not hidden. Since session 17 a date write that crosses the dormant boundary also records the transition in `observed.activityLog` and appends one dated sentence to the row's prose, so a row can never again assert `39 days → DORMANT` over a date that evaluates ACTIVE — a registry-wide test now fails if it does. |
| Verifier drift watch | **Computed, not typed** — `verifierDrift()` in `assets/js/data.js`, rendered in a 🧭 panel on the reporter page. Three kinds of rot under a reporter row are re-measurable for free and now surfaced with per-fact evidence links: a recorded-valid **verification object** now missing or `isValid:false`; a **beat** the account's own bio has moved *or dropped* (`beat-departed` and `beat-changed` are separate labels, because only one of them leaves a club with no claim); and an **activity** transition across the 30-day line. On the committed 2026-09-19 evidence it reports three facts: Andy Larsen's Tribune bio now marks the Jazz beat *former* (**a UTA coverage loss**), Joe Vardon carries an object the API marks invalid (issuer `theathletic.com`, recorded as a standing risk on a row that never graded him on it), and the Grange activity transition. **None of these downgrades an identity** — a revoked object does not make a person someone else, a bio is not a credential, and a quiet account is not a wrong one. |
...
- Official 2026–27 injury-report page: **404** (last re-read 2026-09-19 ~21:51Z, `XID: 79058498` — 3d ago on 2026-09-22; earlier XIDs 71103381 / 44289229 / 74717976 / 72640245). Still no PDF link, and no filename has been guessed. Dashboard now links the refusal with its age rather than restating it as fresh.
...
- ESPN injuries: **200**, 75 rows in 27 team blocks in the live API (21:22Z run; re-read 2026-09-19 ~18:50Z, `timestamp 2026-09-19T18:50:34Z`, same 27 blocks and the same three omissions) and **75 rows / 27 teams / empty CLE-DET-LAL arrays in the committed snapshot `2026-09-20T12:20:59Z`** — **CLE, DET and LAL returned no block at all in the live read and an empty array in the snapshot**, which the board now names explicitly. The team dropdown *does* list all 30, so the omission is in the data, not the navigation.
...
node tools/intelligence_test.js          # reporter-ledger forward scoring: both reconciliation lanes, the stale-stamp
                                      # guard, identity gating, silence-is-pending, official-over-board authority
