...
- `data/audit/latest.json` in this commit was still produced by the **old** tool (it shows the four defects). The next runner
  audit regenerates it with `verified` flags; read the `verifiedByThisRun` counter, not just "no drift".
