...
const out = {
  ...
  method: "Single source of truth: assets/js/data.js. Each entry verified line-by-line against live official pages/APIs on 2026-09-17 across seven passes (verification channels: assistant page-fetch tool + GitHub Actions runner audit data/audit/latest.json; shell HTTPS egress is blocked in the build environment). See sources.html for evidence links. Session 2 added the structured all-30-team ESPN injuries API and the free Bluesky/AT-Protocol social layer; session 4 re-verified the official/ESPN/Bluesky rows and fixed the social alert-eligibility gate (identity evidence, not only the verification object); session 6 resolved the Jake Fischer outlet question from the outlet's own byline and re-verified the Buckner/Haynes/Guillory rows against dated 2025-2026 reports; session 7 re-read the audit tool itself and fixed four of its checks (getList AT-URI parameter, Bluesky verification field, ESPN roster path, and a verdict ladder that printed OK for checks it could not run), then re-verified the 2026-27 official page (still 404), the ESPN injuries and roster payloads, the NBA account's 6 follows (4 valid verification objects) and Howard Beck's 150-member writers list; session 8 re-read the LIVE deployed page line by line (board 75 listings via espn-direct, coverage gaps CLE/DET/LAL named, ESPN news OK, scoreboard 0 events) and fixed the one rendering defect found there (legend printed 'undefined' — see the session-8 FLAGS entry); session 12 re-verified every Bluesky row in the alert path against the keyless API on 2026-09-18 and pinned the two identities that refuse verification (see the identityRefused rows); session 13 (2026-09-18) probed 25 candidate official-club handles with one keyless getProfiles request (16 resolved, 9 do not exist, 0 carry a verification object, 3 carry Bluesky's own impersonation label — see NBA_OFFICIAL_ACCOUNT_PROBE), added the activity model that separates identity from recency (ARENA_DORMANT_DAYS, writerRecency, arenaCoverage().recency), taught the verifier to read Bluesky moderation labels (impersonation-label / profile-private verdicts), and re-documented the Instagram/Facebook read blocker with dated platform evidence; session 17 (2026-09-19) re-read every standing constraint live (official 2026-27 report page still 404, new XID 72640245; ESPN injuries JSON 200 at 2026-09-19T18:50:34Z, 2026-27 Preseason; Basketball Monster playernews.aspx 200 with the 10/20 slate; nba.com on Bluesky valid with 123,008 followers and the Oct 20 tripleheader in its own bio; theathletic.com's object still isValid:false while its trustedVerifierStatus stays valid; rodboone.bsky.social still has no bio on a second dated read), re-probed 41 club handles in four batched keyless getProfiles calls and found ZERO new verification objects (POR, PHI, DEN and the league account remain the only four), and shipped verifierDrift() plus its reporters.html panel so revoked verification objects, beat changes and activity transitions are computed and shown instead of sitting in an audit file; all rows carry their exact evidence strings below.",
  endpoints: ENDPOINTS,
  counts: {
    teams: TEAMS.length,
  ...
  social_layer: {
    ...
    cors_note: "Browser cross-origin behaviour was RESOLVED 2026-09-17 by reading the deployed page itself: the site at buffedlizard55-lab.github.io fetched both site.web.api.espn.com/injuries ('via espn-direct') and public.api.bsky.app (13/13 accounts browser-direct) from the deployed origin, re-verified again in session 8 (2026-09-17 ~23:15Z). Three transport paths remain shipped because a blocking extension, a rate limit or an ESPN policy change can still force the fallback: browser-direct, same-origin CI snapshot (data/live/latest.json), and an explicitly labelled opt-in public relay. The UI prints which path each panel used."
  },
  ...
  key_limitations: [
    ...
    "Browser CORS for the two cross-origin endpoints (site.web.api.espn.com/injuries and public.api.bsky.app) was verified WORKING from the deployed origin by reading the live GitHub Pages page (2026-09-17, re-confirmed session 8); the fallbacks (same-origin CI snapshot, labelled opt-in relay) remain because browser policies can change.",
    ...
    "Instagram/Facebook offer no keyless read path for a page we do not administer (re-checked 2026-09-18): tokenless oEmbed was removed 2020-10-24, the Instagram Basic Display API returns an error for all requests since 2024-12-04, the Instagram API with Facebook Login needs an IG business/creator account linked to a Facebook Page, and the Graph API needs an App Access Token plus the reviewable 'Page Public Content Access' feature (and appsecret_proof since v5.0). Both are therefore published as manual-review links only, never as alert inputs (SOURCES ids instagram-public-pages / facebook-public-pages).",
    "Identity is not activity: 49 of the 54 Bluesky rows are polled, but on the newest-post dates measured 2026-09-18 four franchises (DAL, DEN, LAL, CLE) had no writer who had posted inside 30 days. The per-team activity state is computed and rendered, and data/live/reporter_verify.json is the measurement of record.",
    "Bluesky has no verification object for any official club account probed on 2026-09-18 except POR, DEN, PHI and the league account; three team-name handles carry Bluesky's own 'impersonation' moderation label and are excluded by the verifier, not by opinion.",
