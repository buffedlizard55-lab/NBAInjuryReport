...
## Session 17 (2026-09-19) — two parallel branches, one root defect

> **Two branches audited session 17 in parallel and were merged.** They found the *same* root
> defect — a registry row asserting an activity verdict its own measured date contradicts — from
> different directions. Both audit records are kept below, unedited apart from heading levels,
> rather than blended into one that credits neither.
>
> - **Branch A:** the recency backfill logs transitions and corrects prose; `verifierDrift()` + its
>   reporters.html panel; the prefix+class CSS audit (which found `.pill.dim`); a 41-handle club
>   re-probe.
> - **Branch B:** the negative-day arithmetic clamp; `prose-activity-drift` in the verifier; two more
>   orphaned CSS classes (`.warn`, `.post-head`); the `getAuthorFeed` re-read that proved the stored
>   date was right and the arithmetic wrong.
>
> Where they overlap, both fixes are in the tree. Grange's row carries Branch B's corrected prose
> (it names the post actually read) **and** Branch A's machine-readable `observed.activityLog`, so
> the contradiction is fixed in the prose, recorded in the data, and pinned by a test.
